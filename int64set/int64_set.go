...
// New create and return instance of int64 set
func New(int64s ...int64) *set {
	s := NewWithSize(len(int64s))
	s.Adds(int64s...)
	return s
}
...
func (s *set) Removes(items ...int64) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *set) Exist(item int64) bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *set) List(item int64) []int64 {
	s.mu.RLock()
	defer s.mu.RUnlock()

	i := 0
	l := make([]int64, len(s.data))
